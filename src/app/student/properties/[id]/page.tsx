...
function TenantPropertyView({ property, landlord }: { property: Property, landlord: User | null | undefined}) {
    const { user, isUserLoading } = useUser();
    const firestore = useFirestore();
    const router = useRouter();
    const [isPaymentDialogOpen, setIsPaymentDialogOpen] = useState(false);
  
    const transactionsQuery = useMemoFirebase(() => user ? query(collection(firestore, 'transactions'), where('tenantId', '==', user.uid), where('propertyId', '==', property.id)) : null, [firestore, user, property.id]);
    const { data: transactions, isLoading: areTransactionsLoading } = useCollection<Transaction>(transactionsQuery);

    const leaseQuery = useMemoFirebase(() => user ? query(collection(firestore, 'leaseAgreements'), where('propertyId', '==', property.id), where('tenantId', '==', user.uid)) : null, [firestore, user, property.id]);
...
export default function PropertyDetailPage() {
    ...
    if (isPropertyLoading || isUserLoading || isLandlordLoading || areReviewsLoading) {
        return <TenancySkeleton />;
    }
    ...
    if (user && user.uid === property.currentTenantId) {
        return <TenantPropertyView property={property} landlord={landlord} />;
    }

    return (
        <ProspectiveTenantView 
            property={property} 
            landlord={landlord} 
            reviews={reviews || []} 
            images={images} 
