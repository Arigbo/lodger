...
export default function MessagesPage() {
  ...
  const [localSelectedConversationId, setLocalSelectedConversationId] =
  ...
  const { data: newContact } = useDoc<User>(
    useMemoFirebase(
      () => (contactId ? doc(firestore, "users", contactId) : null),
      [contactId, firestore],
    ),
  );
  ...
  const { data: allStudentMessages, isLoading: messagesLoading } =
    useCollection<Message>(allMessagesQuery);
  ...
  useEffect(() => {
    ...
    const processConversations = async () => {
      ...
      const conversationsMap = new Map<
        string,
        { lastMessage: Message; participantId: string }
      >();
      (allStudentMessages || []).forEach((msg) => {
        const otherParticipantId = msg.participantIds.find(
          (id) => id !== user.uid,
        );
        if (otherParticipantId) {
          if (
            !conversationsMap.has(otherParticipantId) ||
            (msg.timestamp &&
              conversationsMap.get(otherParticipantId)!.lastMessage.timestamp &&
      ...
      const rentedPropertiesQuery = query(
        ...
        where("currentTenantId", "==", user.uid),
        limit(1),
      );
      ...
      const usersQuery = query(
        ...
        where(documentId(), "in", allParticipantIds),
      );
      ...
      const convos = allParticipantIds
        .map((pId) => {
          let participant = usersMap.get(pId);

          if (!participant && pId === "SYSTEM_MODERATION") {
            participant = {
              id: "SYSTEM_MODERATION",
              name: "Lodger Moderation",
              email: "moderation@lodger.com",
              role: "admin",
              profileImageUrl:
                "https://cdn-icons-png.flaticon.com/512/10673/10673062.png", // Security/Moderation icon
            } as User;
          }
          ...
          const unreadCount = (allStudentMessages || []).filter(
            (msg) =>
          ...
          return {
            ...
            lastMessageTimestamp:
              convoData?.lastMessage.timestamp?.toDate() || new Date(0),
            unreadCount,
          } as Conversation;
        })
  ...
  useEffect(() => {
    if (!user || !selectedParticipant || !firestore || !allStudentMessages)
      return;

    const markAsRead = async () => {
      const unreadMessages = allStudentMessages.filter(
        (msg) =>
  ...
  const handleSendMessage = () => {
    if (!newMessage.trim() || !user || !selectedParticipant || !firestore)
      return;
    ...
    addDoc(messagesRef, messageData)
      ...
      .catch((serverError: any) => {
        ...
        alert(
          `Failed to send message: ${serverError.message || "Unknown error"}`,
        );
      });
  };

  const selectedConversation = conversations.find(
    (c) => c.id === localSelectedConversationId,
  );
    ...
    <div className="flex h-[calc(100dvh-7rem)] md:h-[calc(100vh-8rem)] gap-0 md:gap-8 animate-in fade-in duration-700 overflow-hidden -mt-4 md:mt-0">
      ...
      >
        <div className="px-2">
          ...
          <p className="text-[10px] md:text-sm font-medium text-muted-foreground mt-1 text-left">
            Your direct connection to landlords.
          </p>
        </div>
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setIsSidebarOpen(false);
                      router.replace(`${pathname}?conversationId=${conv.id}`, {
                        scroll: false,
                      });
                    }}
                    className={cn(
                      ...
                      <div className="flex justify-between items-start mb-0.5">
                        ...
                        <span className="text-[10px] font-bold text-muted-foreground/40 whitespace-nowrap">
                          {conv.lastMessage?.timestamp
                            ? formatDistanceToNow(
                                new Date(conv.lastMessage.timestamp.toDate()),
                                { addSuffix: false },
                              )
                            : ""}
                        </span>
      ...
      {/* Chat Area */}
      <div className={cn("flex-1 h-full", isSidebarOpen && "hidden md:block")}>
              ...
              <div className="flex items-center gap-3 md:gap-6">
                ...
                <Avatar className="h-10 w-10 md:h-16 md:w-16 rounded-xl md:rounded-[1.25rem] border-2 border-primary/5 shadow-md">
                  <AvatarImage
                    ...
                    className="object-cover"
                  />
                  <AvatarFallback className="bg-muted text-xl md:text-2xl font-black">
                    {selectedConversation?.otherUser?.name?.[0]}
                  </AvatarFallback>
                </Avatar>
                <div className="space-y-0.5">
                  <div className="flex items-center gap-2 md:gap-3">
                    <h2 className="text-base md:text-xl font-black uppercase tracking-tight leading-none truncate max-w-[150px] md:max-w-none">
                      {selectedConversation?.otherUser?.name}
                    </h2>
                    <div className="h-1 md:h-1.5 w-1 md:w-1.5 rounded-full bg-green-500 shadow-[0_0_12px_rgba(34,197,94,0.5)]" />
                  </div>
                  <p className="text-[7px] md:text-[10px] font-bold text-muted-foreground/40 uppercase tracking-[0.1em]">
                    Verified Landlord Connection
                  </p>
                </div>
              </div>
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button
                      variant="ghost"
                      size="icon"
                      className="h-10 w-10 md:h-12 md:w-12 rounded-xl md:rounded-2xl hover:bg-muted/30 transition-all text-muted-foreground"
                    >
                ...
                {Object.entries(
                  ...
                  <div key={date} className="space-y-8">
                    <div className="flex justify-center py-4 relative">
                      ...
                      <span className="relative z-10 px-6 py-2 rounded-full bg-white border-2 border-muted/5 text-[10px] font-black uppercase tracking-[0.2em] shadow-sm">
                        {date}
                      </span>
                    </div>
                          ...
                          <AvatarImage
                            src={
                              msg.senderId === user?.uid
                                ...
                                : selectedParticipant?.profileImageUrl
                            }
                            className="object-cover"
                          />
                          <AvatarFallback className="text-[10px] font-black">
                            {msg.senderId === user?.uid ? "ME" : "U"}
                          </AvatarFallback>
                        </Avatar>
          ...
          <Card className="h-full flex flex-col items-center justify-center rounded-[2rem] md:rounded-[3rem] border-2 border-dashed border-muted/10 bg-muted/5 p-8 md:p-20 text-center space-y-6 md:space-y-8 animate-in zoom-in duration-700">
            ...
            <div className="space-y-2 md:space-y-3">
              <h3 className="text-xl md:text-3xl font-black uppercase tracking-tight">
                Channel <span className="text-primary">Hibernated</span>
              </h3>
