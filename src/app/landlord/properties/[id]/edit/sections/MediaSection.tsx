    ...
    <div className="space-y-12">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {property.images.map((url) => (
            <div key={url} className="relative group aspect-square rounded-[2rem] overflow-hidden border-2 border-foreground/5 shadow-inner">
              <Image src={url} alt="Property" fill className="object-cover transition-transform duration-700 group-hover:scale-110" />
          ...
          <label className="relative flex flex-col items-center justify-center aspect-square rounded-[2rem] border-2 border-dashed border-foreground/10 hover:border-primary/50 hover:bg-primary/5 transition-all cursor-pointer group">
            ...
            <span className="mt-2 text-[10px] font-black uppercase tracking-widest text-muted-foreground group-hover:text-primary">Inject Media</span>
            <input
              type="file"
              multiple
              accept="image/*"
      ...
      <div className="space-y-10">
        <div className="inline-flex items-center gap-4">
          ...
          <h3 className="font-headline text-xl md:text-3xl font-black uppercase tracking-tighter">Walkthrough Stream</h3>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {property.videos?.map((url) => (
            <div key={url} className="relative aspect-video rounded-[2.5rem] overflow-hidden border-2 border-foreground/5 bg-black group">
            ...
            {isUploading ? (
              ...
              <>
                ...
                <span className="mt-4 text-xs font-black uppercase tracking-[0.2em] text-muted-foreground group-hover:text-primary px-8 text-center">
                  Initialize Walkthrough Protocol (MP4/WebM)
                </span>
              </>
            )}
            <input
