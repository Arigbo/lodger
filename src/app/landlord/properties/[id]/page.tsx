...
export default function LandlordPropertyDetailPage() {
  ...
  const propertyRef = useMemoFirebase(
    () => (id ? doc(firestore, "properties", id) : null),
    [firestore, id],
  );
  const { data: property, isLoading: isPropertyLoading } =
    useDoc<Property>(propertyRef);

  const landlordRef = useMemoFirebase(
    () =>
      property?.landlordId
        ? doc(firestore, "users", property.landlordId)
        : null,
    [firestore, property],
  );
  const { data: landlord, isLoading: isLandlordLoading } =
    useDoc<UserProfile>(landlordRef);

  const tenantRef = useMemoFirebase(
    () =>
      property?.currentTenantId
        ? doc(firestore, "users", property.currentTenantId)
        : null,
    [firestore, property],
  );
  const { data: tenant, isLoading: isTenantLoading } =
    useDoc<UserProfile>(tenantRef);
  ...
  const { data: reviews, isLoading: areReviewsLoading } =
    useCollection<PropertyReview>(reviewsQuery);

  const [aggregatedRequests, setAggregatedRequests] = useState<
    AggregatedRequest[]
  >([]);
  ...
  const [selectedRequest, setSelectedRequest] =
    useState<AggregatedRequest | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchRequestsAndApplicants = async () => {
      ...
      const rentalRequests: RentalApplication[] = requestsSnapshot.docs.map(
        (d: any) => ({ ...d.data(), id: d.id }) as RentalApplication,
      );
      ...
      const applicantIds = [
        ...new Set(rentalRequests.map((r) => r.tenantId)),
      ].filter(Boolean);
      ...
      const finalRequests = rentalRequests.map((request) => ({
        request,
        applicant: request.tenantId
          ? usersMap.get(request.tenantId) || null
          : null,
      }));
  ...
  const handleLeaseSigned = async (finalText: string) => {
    if (selectedRequest?.request && property?.leaseTemplate && landlord) {
      ...
      updateDocumentNonBlocking(propRef, {
        status: "occupied",
        currentTenantId: selectedRequest.request.tenantId,
        leaseStartDate,
      });
          ...
          <div className="lg:col-span-2 space-y-10">
            <div className="space-y-6">
              <div className="flex items-center gap-3">
                <Badge
                  variant="secondary"
                  className="bg-primary/10 text-primary border-none px-4 py-1 text-xs font-bold uppercase tracking-widest"
                >
                  {property.type}
                </Badge>
                {property.status === "occupied" ? (
                  ...
                  <Badge
                    variant="secondary"
                    className="bg-green-500/10 text-green-600 px-4 py-1 text-xs font-bold uppercase tracking-widest"
                  >
                    Available
                  </Badge>
                )}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
              {[
                ...
                <div
                  key={i}
                  className="bg-muted/30 p-6 rounded-3xl border border-border/50 flex flex-col items-center text-center group hover:bg-primary/5 hover:border-primary/20 transition-all"
                >
            ...
            <Tabs defaultValue="gallery" className="w-full">
              <TabsList className="w-full justify-start bg-transparent border-b rounded-none px-0 h-auto gap-8 mb-8">
                <TabsTrigger
                  value="gallery"
                  className="text-base font-bold rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent px-0 pb-4"
                >
                  Gallery
                </TabsTrigger>
                <TabsTrigger
              ...
              <TabsContent value="amenities" className="mt-0">
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                  {property.amenities.map((amenity) => (
                    <div
                      key={amenity}
                      className="flex items-center gap-3 p-4 rounded-2xl border bg-card/50 backdrop-blur-sm group hover:border-primary/30 transition-all font-bold"
                    >
                      <div className="bg-primary/10 p-2 rounded-xl text-primary group-hover:bg-primary group-hover:text-white transition-all">
                        {React.createElement(
                          AMENITY_ICONS[amenity] || DEFAULT_ICON,
                          { className: "h-5 w-5" },
                        )}
                      </div>
                ...
                <div className="bg-muted/20 p-8 rounded-[2rem] border-2 border-dashed border-border">
                  <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {property.rules.map((rule) => (
                      <li
                        key={rule}
                        className="flex items-start gap-3 text-muted-foreground font-bold"
                      >
                ...
                {reviews && reviews.length > 0 ? (
                  <div className="space-y-6">
                    {reviews.map((review) => (
                      <div
                        key={review.id}
                        className="p-6 rounded-3xl border bg-card/50 backdrop-blur-sm space-y-4"
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-4">
                            <Avatar className="h-12 w-12 border-2 border-background shadow-lg">
                              <AvatarImage
                                src={review.tenantImageUrl}
                  ...
                  <div className="text-center py-12 bg-muted/20 rounded-3xl border border-dashed flex flex-col items-center gap-3">
                    ...
                    <p className="text-muted-foreground font-black uppercase tracking-widest text-xs opacity-40">
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <Badge
                  variant="secondary"
                  className="px-3 py-1 bg-primary text-white font-black"
                >
                  {aggregatedRequests.length}
                </Badge>
              </div>
                    ...
                    <Table>
                      <TableHeader className="bg-foreground/5">
                        <TableRow className="hover:bg-transparent">
                          <TableHead className="h-14 font-black text-[10px] uppercase tracking-widest text-foreground pl-8">
                            Applicant
                          </TableHead>
                          <TableHead className="h-14 font-black text-[10px] uppercase tracking-widest text-foreground text-center">
                            Protocol Status
                          </TableHead>
                          <TableHead className="h-14 font-black text-[10px] uppercase tracking-widest text-foreground text-right pr-8">
                            Actions
                          </TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {aggregatedRequests.length === 0 ? (
                          ...
                          aggregatedRequests.map(({ request, applicant }) => (
                            <TableRow
                              key={request.id}
                              className="group hover:bg-primary/[0.02] transition-colors border-b-2 border-foreground/[0.02] last:border-0 h-24"
                            >
                              <TableCell className="pl-8">
                                <div className="flex items-center gap-4">
                                  ...
                                  <div>
                                    ...
                                    <p className="text-[10px] font-bold text-muted-foreground/60 tracking-widest lowercase">
                                      {applicant?.email}
                                    </p>
                                  </div>
                                </div>
              ...
              <Card className="border-none bg-background shadow-[0_0_50px_-12px_rgba(0,0,0,0.12)] rounded-[2.5rem] overflow-hidden">
                <CardHeader className="bg-[#0A0A0A] text-white p-8">
                  <div className="flex items-center justify-between mb-2">
                    <Badge
                      variant="secondary"
                      className="bg-white/10 text-white border-white/20 px-3 py-0.5 text-[10px] font-black uppercase tracking-widest"
                    >
                      Monthly Rent
                    </Badge>
                ...
                <CardContent className="p-8 space-y-6">
                  ...
                  <Button
                    asChild
                    className="w-full h-14 rounded-2xl text-base font-black uppercase tracking-widest shadow-xl shadow-primary/20 hover:scale-[1.02] transition-transform"
                  >
                ...
                <Card className="border-none bg-muted/30 rounded-[2rem] p-8 hover:bg-muted/40 transition-colors group">
                  <div className="space-y-6">
                    ...
                    <div className="flex items-center gap-4">
                      <Avatar className="h-20 w-20 shadow-xl border-4 border-white group-hover:scale-105 transition-transform">
                        <AvatarImage
                          ...
                          className="object-cover"
                        />
                        <AvatarFallback className="bg-foreground text-white text-xl font-black">
                          {tenant.name[0]}
                        </AvatarFallback>
                      </Avatar>
                      <div>
                        ...
                        <p className="text-xs font-bold text-muted-foreground/60 tracking-widest uppercase truncate max-w-[150px]">
                          {tenant.email}
                        </p>
                      </div>
                    </div>
                    <Button
                      asChild
                      variant="outline"
                      className="w-full h-12 rounded-xl border-2 font-black text-xs uppercase tracking-widest"
                    >
